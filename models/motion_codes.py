...
def rgb_flow(a, b):
    if a.dtype != np.uint8 or a.shape != b.shape:
        raise ValueError("Matching uint8 HWC RGB required")
...
def flow_descriptor(f, grid=4):
    ...
    return (
        np.median(tiles.reshape(grid, grid, -1, 2), axis=2)
        .reshape(-1)
        .astype(np.float32)
    )
...
def distances(x, c):
    return ((x[:, None] - c[None]) ** 2).mean(-1)


def assign_codes(x, c):
    return distances(np.asarray(x), c).argmin(1)
...
def fit_codebook(x, codes=8, seed=0, iterations=100, restarts=8):
    ...
    if x.ndim != 2 or len(x) < codes or not np.isfinite(x).all():
        raise ValueError("Invalid training descriptors")
    ...
    for _ in range(restarts):
        ...
        for _ in range(iterations):
            ...
            new = np.stack(
                [
                    x[ids == k].mean(0) if (ids == k).any() else x[d.min(1).argmax()]
                    for k in range(codes)
                ]
            )
            done = np.allclose(new, c, atol=1e-5)
            c = new
            if done:
                break
...
def code_metrics(ids, codes):
    p = np.bincount(ids, minlength=codes) / len(ids)
    h = float(-(p * np.log(p.clip(1e-12))).sum())
