...
import argparse, copy, hashlib, json, os, time, subprocess
...
def image_array(x):
    return (
        ((x.detach().float().clamp(-1, 1) + 1) * 127.5)
        .byte()
        .permute(0, 2, 3, 1)
        .cpu()
        .numpy()
...
class Pilot:
    def __init__(self, a):
        ...
        if not 1 <= self.max_generated_context <= 16:
            raise ValueError("generated_context_max must be between 1 and 16")
        span = self.stride * 4 + 1
        self.train_starts = np.arange(
            300,
            46000
            - span
            + 1
            - (
                self.stride * self.max_generated_context
                if getattr(a, "generated_context_probability", 0) > 0
                else 0
            ),
        )
        self.val = np.linspace(47000, 53000 - span - 1, a.eval_clips, dtype=int)
        self.rng = np.random.default_rng(a.seed)
        self.model = PixelDenoiser(
            len(self.centers),
            4,
            a.width,
            getattr(a, "offset_noise", 0.0),
        ...
        self.opt = torch.optim.AdamW(
            self.model.parameters(), lr=a.lr, weight_decay=0.01
        )
        self.start_step = 0
        if a.resume:
            ck = torch.load(a.resume, map_location=self.device, weights_only=True)
            self.model.load_state_dict(ck["model"])
            self.ema.load_state_dict(ck["ema"])
            self.opt.load_state_dict(ck["optimizer"])
            self.start_step = ck["step"]
            if "rng_numpy" in ck:
                self.rng.bit_generator.state = ck["rng_numpy"]
            if "rng_torch" in ck:
                torch.set_rng_state(ck["rng_torch"].cpu())
            if self.device.type == "cuda" and ck.get("rng_cuda"):
                torch.cuda.set_rng_state_all([x.cpu() for x in ck["rng_cuda"]])
        self.config = {
            **vars(a),
            ...
            "validation_controls": "future-RGB-inferred oracle for paired metrics; direct integer interventions separately",
        ...
        for source in self.config["source_sha256"]:
            (self.out / "source_at_run" / Path(source).name).write_text(
                Path(source).read_text()
            )
        (self.out / "config.json").write_text(json.dumps(self.config, indent=2))
        (self.out / "validation_starts.json").write_text(json.dumps(self.val.tolist()))
        self.run = None
        self.wb = None
        if a.key_file:
            ...
            self.run.define_metric("validation/*", step_metric="step")
            self.run.watch(self.model, log="all", log_freq=1000, log_graph=False)
            print("W&B", self.run.url, flush=True)
            for split, indices in [
                ("train", self.train_starts),
                ("validation", self.val),
            ]:
                stats = code_metrics(
                    self.ids[torch.as_tensor(indices)].numpy(), len(self.centers)
                )
                self.run.log(
                    {
                        f"codes/{split}_entropy": stats["entropy_nats"],
                        **{
                            f"codes/{split}_{k}_fraction": v
                            for k, v in enumerate(stats["fractions"])
                        },
                    }
                )
    ...
    def clips(self, starts):
        ...
        if self.a.conditioning == "constant":
            act = torch.zeros_like(act)
        return x[:, :4], x[:, 4], act

    @torch.no_grad()
    def training_batch(self, starts):
        probability = getattr(self.a, "generated_context_probability", 0.0)
        if not probability or self.rng.random() >= probability:
            return (*self.clips(starts), 0)
        count = int(self.rng.integers(1, getattr(self, "max_generated_context", 2) + 1))
        ix = (
            torch.as_tensor(starts)[:, None]
            + torch.arange(5 + count)[None] * self.stride
        )
        x = self.data[ix].to(self.device).float().permute(0, 1, 4, 2, 3) / 127.5 - 1
        codes = self.ids[ix[:, :-1]].to(self.device)
        if self.a.conditioning == "constant":
    ...
    def evaluate(self, step):
        ...
        for j in range(0, len(self.val), 16):
            ...
            pred = self.ema.sample(
                h, act, self.a.sample_steps, seed=700 + j, heun=self.a.solver == "heun"
            )
            shuffled = act.clone()
            shuffled[:, -1] = act[:, -1].roll(1)
            wrong = self.ema.sample(
                h,
                shuffled,
                self.a.sample_steps,
                seed=700 + j,
            ...
            for name, p in [
                ...
                for k, v in vals.items():
                    sums.setdefault(f"validation/{name}_{k}", []).extend(
                        v.cpu().tolist()
                    )
        ...
        row["validation/shuffle_l1_gap"] = (
            row["validation/shuffled_l1"] - row["validation/inferred_l1"]
        )
        row["validation/generated_motion_code_agreement"] = float(
        ...
        tile = np.concatenate(
            [np.concatenate(image_array(t), axis=1) for t in saved], axis=0
        )
        path = self.out / f"predictions_{step:06}.png"
        Image.fromarray(tile).resize((1024, 640), Image.Resampling.NEAREST).save(path)
        if self.run:
            self.run.log(
                {
                    "samples/previous_target_inferred_shuffled_constant": self.wb.Image(
                        str(path)
                    ),
                    "step": step,
                }
    ...
    def intervene(self, step):
        ...
        for k in range(len(self.centers)):
            ...
            act[:, -1] = k
            p = self.ema.sample(
                h, act, self.a.sample_steps, seed=808, heun=self.a.solver == "heun"
            )
            rows.append(np.concatenate(image_array(p), axis=1))
            ds = np.stack(
                [
                    flow_descriptor(rgb_flow(x, y))
                    for x, y in zip(image_array(h[:, -1]), image_array(p))
                ]
            )
            measured.append(ds.tolist())
        Image.fromarray(np.concatenate(rows, 0)).resize(
            (512, len(rows) * 128), Image.Resampling.NEAREST
        ).save(self.out / f"interventions_{step:06}.png")
        (self.out / f"interventions_{step:06}.json").write_text(
            json.dumps(
                {
                    "source_starts": starts.tolist(),
                    "generated_flow_descriptors_by_requested_code": measured,
                }
            )
        )
        if self.run:
            self.run.log(
                {
                    "samples/direct_code_interventions": self.wb.Image(
                        str(self.out / f"interventions_{step:06}.png")
                    ),
                    "step": step,
                }
            )
    ...
    def checkpoint(self, step):
        state = {
            ...
            "step": step,
            "rng_numpy": self.rng.bit_generator.state,
    ...
    def train(self):
        ...
        last_completed = self.start_step
        try:
            for step in range(self.start_step + 1, self.a.steps + 1):
                if time.time() - self.started > self.a.max_seconds:
                    status = "time_limit"
                    break
                ix = self.rng.choice(self.train_starts, self.a.batch)
                h, y, act, generated_count = self.training_batch(ix)
                ...
                assert torch.isfinite(gn), "Nonfinite gradients"
                self.opt.step()
                with torch.no_grad():
                    for ep, p in zip(self.ema.parameters(), self.model.parameters()):
                        ep.lerp_(p, 1 - min(0.995, (step + 1) / (step + 10)))
                last_completed = step
                ...
                if step % self.a.log_every == 0:
                    ...
                    self.log(
                        step,
                        {
                            "train/loss": float(np.mean(interval)),
                            "train/generated_context_frames_latest": generated_count,
                            "train/generated_context_frames_mean": float(
                                np.mean(generated_counts)
                            ),
                            "train/generated_context_batch_fraction": float(
                                np.mean(np.array(generated_counts) > 0)
                            ),
                            "train/gradient_norm": float(gn),
                            "train/updates_per_second": len(interval) / (now - last),
                            "train/peak_memory_gb": (
                                torch.cuda.max_memory_allocated() / 1e9
                                if self.device.type == "cuda"
                                else 0
                            ),
                            **{"train/" + k: v for k, v in detail.items()},
                        },
                    )
                    interval = []
                    generated_counts = []
            ...
            self.checkpoint(last_completed)
            ...
            if self.run:
                self.run.finish(exit_code=0 if status != "failed" else 1)
